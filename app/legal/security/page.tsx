...
import { Shield, Lock, Check, Mail, Gamepad2 } from "lucide-react";
    ...
    <main className="min-h-screen bg-[#0a0a0a] overflow-hidden">
      ...
      <section className="relative z-10 pt-20 pb-24 sm:pt-28 sm:pb-32">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <Shield className="mx-auto w-16 h-16 text-violet-400 mb-6" />
          <h1 className="text-5xl md:text-6xl font-extrabold bg-gradient-to-r from-violet-400 via-fuchsia-400 to-cyan-400 bg-clip-text text-transparent mb-6">Security at MultiGen</h1>
          <p className="text-2xl text-white/80 mb-8 font-medium max-w-2xl mx-auto">
            Your trust is our top priority. We use advanced security practices to keep your data and experience safe at every step.
          </p>
        </div>
      </section>
      {/* Main Content */}
        ...
        <div className="grid gap-10 md:grid-cols-2">
          <div className="bg-white/10 border border-violet-500/30 rounded-3xl p-8 shadow-2xl">
            <div className="flex items-center gap-4 mb-3">
              ...
              <h2 className="text-2xl font-bold text-white">End-to-End Encryption</h2>
            </div>
            <p className="text-lg text-white/70">All sensitive data is encrypted in transit and at rest using industry-leading protocols.</p>
          </div>
          <div className="bg-white/10 border border-fuchsia-500/30 rounded-3xl p-8 shadow-2xl">
            <div className="flex items-center gap-4 mb-3">
              <Check className="w-8 h-8 text-fuchsia-400" />
              <h2 className="text-2xl font-bold text-white">Continuous Audits</h2>
            </div>
            <p className="text-lg text-white/70">We regularly audit our systems and codebase to proactively identify and resolve vulnerabilities.</p>
          </div>
          <div className="bg-white/10 border border-cyan-500/30 rounded-3xl p-8 shadow-2xl">
            <div className="flex items-center gap-4 mb-3">
              <Shield className="w-8 h-8 text-violet-400" />
              <h2 className="text-2xl font-bold text-white">Account Protection</h2>
            </div>
            <p className="text-lg text-white/70">Multi-factor authentication and advanced monitoring help keep your account secure from unauthorized access.</p>
          </div>
          <div className="bg-white/10 border border-violet-500/30 rounded-3xl p-8 shadow-2xl">
            <div className="flex items-center gap-4 mb-3">
              <Mail className="w-8 h-8 text-cyan-400" />
              <h2 className="text-2xl font-bold text-white">Report a Vulnerability</h2>
            </div>
            <p className="text-lg text-white/70">If you discover a security issue, please contact <a href="mailto:security@multigen.com" className="underline text-cyan-300">security@multigen.com</a>. We appreciate your help in keeping MultiGen safe for everyone.</p>
          </div>
        </div>
      </section>
      {/* Footer */}
